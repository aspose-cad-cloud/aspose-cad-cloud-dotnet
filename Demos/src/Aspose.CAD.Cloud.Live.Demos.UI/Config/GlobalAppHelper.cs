...
namespace Aspose.CAD.Cloud.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeCADCloudContext.atcc = new AsposeCADCloudContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeCADCloudContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeCADCloudContext.Current.Cache.Add(
				   SessionID,
					resources,
